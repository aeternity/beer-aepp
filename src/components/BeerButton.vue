<template>
  <div class="buy shell" v-bind:class="{orderDone: isOrderDone}">
    <div class="buyButton" v-if="hasTokensForBeer && beerAvailable && t">
      ...
      >Use your tokens (upper right hand corner) to order a beer at our stand.</p>
      <!-- <ae-label for="numBeers" :help-text="errors.first('numBeers')">How many beers?</ae-label> -->
      <ae-input
        id="numBeers"
        name="numBeers"
        type="number"
        v-model.number="selectedBeerNumber"
        v-validate="`min_value:1|max_value:${maxBeers}`"
        units="beers"
      >
      ...
      <ae-modal-light v-if="modalVisible == true" @close="modalVisible = false">
        ...
        <div class="row" style="display:flex;align-items:center;">
          <div>
            <p class="modal-address-name">{{account.name}}</p>
          </div>
          <div></div>
          <div>
            <p class="modal-address-name">æternity bar</p>
          </div>
        </div>
        <ae-divider/>
        <div class="summary">
          ...
          <h5 class="modal-beer-count">{{selectedBeerNumber}} beer(s)</h5>
        </div>
        <ae-divider/>
        <div class="fees">
          <div>
    ...
    <div v-if="ajaxCall.status=='idle'" class="link fillBeer">
      <h1 class="screen-title" style="margin-top:10vh;">🍺</h1>
      <h1>We are filling up your beer!</h1>
      <p
        class="loadingText"
      >Head over to the æternity stand, have a drink and chat with us! We are in the main hall next to the wardrobe, near the main entrance.</p>
      <p
        class="loadingText"
      >At this moment, your transaction on the blockchain is being confirmed by being included in a newly mined block.</p>
      <div id="water">
        <div class="wave">
          <svg xmlns="http://www.w3.org/2000/svg" width="1600" height="198">
            <defs>
              <linearGradient id="a" x1="50%" x2="50%" y1="-10.959%" y2="100%">
                <stop stop-color="#57BBC1" stop-opacity=".25" offset="0%"></stop>
    ...
    <div class="link" v-if="!hasTokensForBeer && ajaxCall.status !=='ready'">
      ...
      <p
        class="screen-subtitle"
      >You don’t have enough tokens to buy a beer. To let someone transfer some tokens to you, simply click on ‘Receive’ below.</p>
    ...
    <div class="link" v-if="barClosed">
      ...
      <p
        class="screen-subtitle"
      >Sorry, there is no one to serve your beer right now. Our stand is only open for beer service from 12:00&hairsp;–&hairsp;19:00 on May 2nd, 3rd, and 4th.
        <br>We are in the main hall next to the wardrobe, near the main entrance.
      </p>
    </div>
    <div class="link" v-if="beerEmpty">
      <h1 class="screen-title">🍺</h1>
      <p class="screen-subtitle">Sorry. Currently the Bar is out of beer.</p>
    </div>
  </div>
</template>
